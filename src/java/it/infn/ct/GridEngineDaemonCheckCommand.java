...
/**
 * Runnable class responsible to check GridEngineDaemon commands This class
 * mainly checks for commands consistency and it does not handle directly
 * GridEngine API calls but rather uses GridEngineInterface class instances The
...
public class GridEngineDaemonCheckCommand implements Runnable {
     ...
     * GridEngineDaemon database connection URL, necessary to finalize executed
     * commands
     *
     * @param gedCommand
     * @param gedConnectionURL
     */
    public GridEngineDaemonCheckCommand(GridEngineDaemonCommand gedCommand, String gedConnectionURL) {
     ...
     * tagged as FAILED Check job status verifies the job status inside the
     * GridEngine' ActiveGridInteraction Check on GE, verifies that job has been
        ...
        if (gedCommand.getStatus().equals("PROCESSING")) {
            ...
            if (gedCommand.getAGIId() != 0) {
                ...
                _log.debug("Status of job "
                        + gedCommand.getTaskId() + " is '"
                        + geJobStatus + "'");
                gedCommand.setGEStatus(geJobStatus);
